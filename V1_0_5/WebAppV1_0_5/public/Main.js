        ...
        const setupUI = (user) => {
          if (user) {
            ...
            dbRefLat.on('value', snap => {
              latElement.innerText = snap.val().toFixed(2);
            });

            dbRefLng.on('value', snap => {
              lngElement.innerText = snap.val().toFixed(2);
            });

            dbRefSpeed.on('value', snap => {
              speedElement.innerText = snap.val().toFixed(2);
            });
            dbRefSos.on('value', snap => {
                sosElement.innerText = snap.val().toFixed(2);
              });
